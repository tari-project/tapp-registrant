...
import { createBranch, createPullRequest, initOctokitAndGetAuthUser } from "../helpers/repo.js"
...
export async function deprecateTappVersion(version: string) {
  ...
  const filePath = path.join(SRC_DIR, manifest.packageName, VER_DIR, manifest.version, MANIFEST_FILE)
  try {
  ...
  const manifestFileContent = Buffer.from(JSON.stringify(manifest)).toString("base64")
